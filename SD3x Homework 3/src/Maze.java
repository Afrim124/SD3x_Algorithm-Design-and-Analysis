...
public class Maze {
	...
	public Graph g; //We will store the maze internally as a graph
	...
	public Maze(String filename) throws IOException{
		...
		for (int row = 0; row < n; row++) {
			for (int col = 0; col < n; col++) {
				int colNumber = row*n + col;
				int e = grid[row][col];
				if ( e == 1 | e == 2 | e == 3) {
					if (col+1<n && (grid[row][col+1] == 1 || grid[row][col+1] == 3)) {
						g.addEdgeForMaze(row*n+col, row*n+col+1);
						g.addDescription(colNumber, Move.RIGHT);
					}
					if (col>=1 && (grid[row][col-1] == 1 || grid[row][col-1] == 3)) {
						g.addEdgeForMaze(row*n+col, row*n+col-1);
						g.addDescription(colNumber, Move.LEFT);
					}
					if (row+1<n && (grid[row+1][col] == 1 || grid[row+1][col] == 3)) {
						g.addEdgeForMaze(row*n+col, (row+1)*n+col);
						g.addDescription(colNumber, Move.DOWN);
					}
					if (row>=1 && (grid[row-1][col] == 1 || grid[row-1][col] == 3)) {
						g.addEdgeForMaze(row*n+col, (row-1)*n+col);
						g.addDescription(colNumber, Move.UP);
					}
				}
			}
		}
	...
	public List<Move> solveMaze() {
		Graph.DepthFirstSearch DFS = this.g.new DepthFirstSearch(g);
		Graph.Node startNode = this.g.new Node(startVertex);
		Graph.Node endNode = DFS.getNode((Integer)endVertex);
		boolean found = DFS.dfs(endNode, startNode);
		if(found) {
			return DFS.getNodesVisited();
		}
		return null;
	}
...
//		Maze maze = null;
//		try {
//		 maze = new Maze("Maze.txt.txt") ;
//		} catch (IOException e1) {
//			System.out.println("File doesn't exist.");
//		}
//		boolean found = false;
//		List<Move> moveList = new ArrayList<Move>();
//		moveList = maze.solveMaze();
//		if ( moveList != null)
